...
// Normalize: uppercase, no diacritics, ALL punctuation → space, collapse spaces
...
Deno.serve(async (req) => {
  try {
    ...
    // ── Phase 1: Group schools by normalized key ───────────────────────────────
    ...
    const nameToCanonical = new Map(); // nd(name) → canonical school
    const canonicalSchools = [];
    const toDelete = [];

    for (const [normKey, group] of byNorm.entries()) {
      if (group.length === 1) {
      ...
      // Pick canonical: most linked groups, then cleaner name (no punctuation), then shortest
      ...
      console.log(`[MERGE] "${dups.map(d=>d.name).join('", "')}" → "${canonical.name}"`);
    }
    ...
    for (const dup of toDelete) {
      ...
      for (const g of affectedGroups) {
        ...
        g.school_name = canon.name; g.school_id = canon.id;
    ...
    if (groupUpdates.length > 0) { await seqUpdates(base44.entities.Group, groupUpdates, 380); await sleep(400); }
    if (regUpdates.length > 0) { await seqUpdates(base44.entities.Registration, regUpdates, 350); await sleep(400); }
    ...
    // ── Phase 3: Recalculate unique participants per school ───────────────────
    const schoolUpdates = [];
    ...
    for (const school of canonicalSchools) {
      ...
      const totalEntries = allP.length;

      // Warn: same name, multiple distinct birth dates (both non-empty) → dubious
      ...
      for (const [name, dates] of byName.entries()) {
        if (dates.size > 1) {
          log.warnings.push(`⚠️ CASO DUDOSO — ${school.name}: "${name}" con fechas distintas: ${[...dates].join(" / ")}`);
        }
      }

      // No birth date → count individually, log as warning
      const noBirth = allP.filter(p => !p.birth);
      for (const p of noBirth) {
        log.warnings.push(`⚠️ SIN FECHA — ${school.name}: "${p.name}" sin fecha de nacimiento, contado individualmente`);
      }

      // Deduplicate: key = name|birth. No-birth participants each get a unique key via index
      const seen = new Set();
      let uniq = 0;
      allP.forEach((p, idx) => {
        const key = p.birth ? `${p.name}|${p.birth}` : `__nobirth__${idx}__${p.name}`;
        if (!seen.has(key)) { seen.add(key); uniq++; }
      });

      schoolUpdates.push({ id: school.id, data: { unique_participants: uniq } });
      log.participantsTable.push({
        school: school.name,
        totalEntries,
        uniqueParticipants: uniq,
        difference: totalEntries - uniq,
    ...
    // ── Phase 4: Persist updates & snapshot ───────────────────────────────────
    ...
    console.log(`[INFO] Done — Merged: ${log.schoolsMerged.length}, Ok: ${log.schoolsOk.length}, Warnings: ${log.warnings.length}`);
